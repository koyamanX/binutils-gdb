...
void
sun32_cgen_print_operand (CGEN_CPU_DESC cd,
			   int opindex,
			   void * xinfo,
			   CGEN_FIELDS *fields,
			   ...
			   int length)
{
  ...
  switch (opindex)
    ...
    case SUN32_OPERAND_RB :
      print_keyword (cd, info, & sun32_cgen_opval_h_gr, fields->f_rb, 0);
      break;
...
cgen_print_fn * const sun32_cgen_print_handlers[] =
{
  print_insn_normal,
};
...
void
sun32_cgen_init_dis (CGEN_CPU_DESC cd)
{
  sun32_cgen_init_opcode_table (cd);
  sun32_cgen_init_ibld_table (cd);
  cd->print_handlers = & sun32_cgen_print_handlers[0];
  cd->print_operand = sun32_cgen_print_operand;
}
		   ...
		   int length)
{
  ...
  for (syn = CGEN_SYNTAX_STRING (syntax); *syn; ++syn)
      ...
      if (CGEN_SYNTAX_CHAR_P (*syn))
	{
      ...
      /* We have an operand.  */
      sun32_cgen_print_operand (cd, CGEN_SYNTAX_FIELD (*syn), info,
...
int
print_insn_sun32 (bfd_vma pc, disassemble_info *info)
{
...
#define CGEN_BFD_ARCH bfd_arch_sun32
#endif
  arch = info->arch;
  ...
  if (! cd)
	...
	abort ();
      ...
      prev_mach = mach;
      prev_endian = endian;
      cd = sun32_cgen_cpu_open (CGEN_CPU_OPEN_ISAS, prev_isa,
